		...
		if (response != null) {
			try {
				...
				for (int i = 0; i < arr.length(); i++) {
					String streamId = "" + arr.getJSONObject(i).getInt("_id");
					String viewers = "" + arr.getJSONObject(i).getInt("viewers");
					JSONObject channel = arr.getJSONObject(i).getJSONObject("channel");
					if (channel != null) {
						String game = channel.get("game").toString().replace(";", "");
						String name = channel.get("name").toString().replace(";", "");
						String status = channel.get("status").toString().replace(";", "");
						String mature = channel.get("mature").toString().replace(";", "");
						String updated = channel.get("updated_at").toString().replace(";", "");
